...
> 以下内容提炼自设计深潜系列，阐述对话引擎与消息流背后的设计理念。

### 这不是聊天引擎，而是执行内核

Claude Code 的对话引擎最值得称道的地方，不是它能"边流式输出边调工具"，而是它把模型调用、工具执行、上下文压缩、错误恢复和预算控制，统一进了一个可持续迭代的主循环里。它像操作系统里的 scheduler，决定什么时候给模型、什么时候给工具、什么时候压缩历史、什么时候恢复错误。

### 核心抽象不是消息，而是"迭代状态"

`queryLoop()` 区分了不会在本轮变化的 immutable params 和会跨迭代推进的 mutable state。好处：

- 所有 continue/retry 点都知道该恢复哪些状态
- 未来如果需要把对话内核 reducer 化，迁移成本很低

好的实现会优先为**"失败后怎么继续"**设计数据结构。

### 压缩链为什么是 snip → microcompact → autocompact

这个顺序背后有很强的工程理性：

1. **越晚做的压缩越不可逆**。snip 删除冗余尾迹，microcompact 压扁可保留结构的内容，autocompact 才做真正的摘要化重写。
2. **压缩不是只看 token 数**。如果前两者已经压到阈值之下，就没必要进入摘要化。追求的是"足够短且尽量原汁原味"。
3. **压缩结果是双账本结构**。context collapse 不是把 summary 直接写回消息数组，而是保存在 collapse store、读时重放。用户拥有完整历史，模型吃到被折叠过的视图。

### 工具结果预算先于压缩

tool result 往往是上下文膨胀的头号来源，但它和普通对话文本不同：有强结构、强引用关系，还要支持 resume。先把"结果太大"单独消化，再送进压缩链。半结构化数据应该**先按自己的规则瘦身，再进入通用压缩**。

### 工具执行是主循环的内生阶段

模型与工具并不是主从关系，而是**交替推进同一个任务状态**。这解释了：

- 为什么要在 tool batch 之后生成 tool use summary
- 为什么 subagent 可以复用同样的 query loop
- 为什么 hook、预算、压缩都必须看"完整迭代"而不是"单次 API 调用"

### 错误恢复的重点是保住循环

Claude Code 的思路：尽量让失败变成"状态迁移"，而不是"流程终止"。比如 token 超限，不是立刻崩，而是先尝试提升输出上限、再恢复计数、再触发额外压缩、只有超过恢复上限后才中止。

用户想要的是任务完成，不是错误透明性本身。只要系统还能用语义上合理的方式继续推进，它就不应该把偶发失败直接暴露为终止。**RPC 的正确性靠立即失败；agent 的正确性靠可控恢复。**
