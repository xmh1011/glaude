...
3. **读 Claude Code 源码**：读 `/Users/xiaominghao/code/claude-code` 中对应模块源码，理解设计意图
...
- `src/query.ts` — agentic loop 退出条件和错误恢复
- `src/tools/BashTool/prompt.ts` — 生产事故提炼的 prompt（6500 tokens，比代码重要）
- `src/tools/` — 每个工具的 `inputSchema` 和 `description`（LLM 的工具说明书）
- `src/services/compact/` — 三层压缩的触发条件和保留策略
...
- [ ] 定义通用 `Message` 与 `ContentBlock` 模型
- [ ] 定义 `Provider` 接口，实现 `AnthropicProvider` (带错误解析)
- [ ] 实现 `MockProvider`，读取本地静态 JSON 剧本以供低成本测试
- [ ] 实现基础 Agent 状态机 (`while` 循环：请求 -> 判断 `stop_reason` -> 退出)

### Phase 2: 核心工具与持久化沙箱
> 📖 开始前读：`docs/plan.md` Phase 2 + `docs/reference/02-tool-system.md` + `docs/reference/06-bash-engine.md`
- [ ] 定义带 `ctx` 的 `Tool` 接口，实现 `Registry` 注册表
- [ ] 实现 `FileReadTool` 与 `FileEditTool` (精准 str_replace)
- [ ] 实现持久化 `BashTool` (后台 `bash --norc` 进程，基于 UUID Sentinel 截断输出)
- [ ] 将工具执行逻辑整合进 Agent 主循环，并将执行错误作为上下文回调给 LLM
